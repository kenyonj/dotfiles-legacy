...
def main(argv):
    ...
    if config.get('oauth2', 'client_id_eval'):
        client_id = eval(config.get('oauth2', 'client_id_eval'))
    else:
        client_id = config.get('oauth2', 'client_id')

    if config.get('oauth2', 'client_secret_eval'):
        client_secret = eval(config.get('oauth2', 'client_secret_eval'))
    else:
...
def build_accounts(config):
    ...
    for section in acct_sections:
        ...
        if config.get(section, 'refresh_token_eval'):
            refresh_token = eval(config.get(section, 'refresh_token_eval'))
        else:
