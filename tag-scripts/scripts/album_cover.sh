...
if [[ "$song_source" == "spotify" ]]; then
  image_url=$(curl -X GET "$spotify_api_url" | ~/.local/bin/jsawk 'return this.album.images[0].url')
  curl "$image_url" -o /tmp/cover.jpg
else
  result_count=$(curl -X GET "$itunes_api_url" | ~/.local/bin/jsawk 'return this.resultCount')

  if [[ "$result_count" > 0 ]]; then
    image_url=$(curl -X GET "$itunes_api_url" | ~/.local/bin/jsawk 'return this.results[0].artworkUrl100')
    curl "$image_url" -o /tmp/cover.jpg
  else
    result_count=$(curl -X GET "$alt_itunes_api_url" | ~/.local/bin/jsawk 'return this.resultCount')

    if [[ "$result_count" > 0 ]]; then
      image_url=$(curl -X GET "$alt_itunes_api_url" | ~/.local/bin/jsawk 'return this.results[0].artworkUrl100')
      curl "$image_url" -o /tmp/cover.jpg
    else
